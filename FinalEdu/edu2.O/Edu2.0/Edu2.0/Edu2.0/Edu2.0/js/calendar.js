...
function getEventCategoryClass(event) {
    const category = String(event.category || '').trim().toLowerCase();
    if (category.includes('academic')) return 'academic';
    ...
    if (color === '#3b82f6' || color === '#3498db' || color === '#00d2ff' || color === '#ff79c6') return 'academic';
    if (color === '#10b981' || color === '#2ecc71' || color === '#55efc4' || color === '#8be9fd') return 'personal';
    if (color === '#bc4ca0' || color === '#9b59b6' || color === '#a29bfe' || color === '#50fa7b') return 'social';
    if (color === '#ef4444' || color === '#ff5555') return 'urgent';
...
function renderCalendar() {
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        dayEventsData.forEach((e) => {
            ...
            const categoryClass = getEventCategoryClass(e);
            if (categoryClass) ev.classList.add(categoryClass, `event-${categoryClass}`);
...
async function saveEvent() {
    ...
    if (!title) return alert('Give your event a title so we can save it nicely. 🗓️');
    if (!token) return alert('Please sign in first to keep your calendar synced. 🔐');
    ...
    alert("Oops! We couldn't save that event right now. Let's try again. 🛠️");
}
