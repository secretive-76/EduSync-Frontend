...
const API_BASE = getApiUrl('/api/academic');
...
async function saveSemesterToCloud() {
    ...
    if (!semesterName) {
        alert('Please add a semester name so your record stays organized. 📚');
        return;
    }

    const calc = calculateEverything();
    if (calc.courses.length === 0) {
        alert('Add at least one course before saving your semester snapshot. ✍️');
        return;
    }
    ...
    try {
        ...
        await loadAcademicData();
        alert('Your semester is saved and synced! Looking great. ☁️✅');
    } catch (error) {
        alert("Oops! We hit a small snag while saving the semester. Let's try again. 🛠️");
    }
}
...
async function deleteSemester(semesterId) {
    ...
    try {
        ...
        alert("We couldn't remove that semester just now. Please try once more. 🛠️");
    }
}
...
async function calculateNextSemRequirement() {
    ...
    if (isNaN(targetCGPA) || isNaN(nextCredits) || nextCredits <= 0) {
        alert("Please enter a valid target CGPA and next-semester credits so we can calculate accurately. 🎯");
        return;
    }
