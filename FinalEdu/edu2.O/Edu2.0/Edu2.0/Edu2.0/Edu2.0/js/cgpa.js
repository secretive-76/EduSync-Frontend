...
async function saveCourseToCloud(courseId) {
    ...
    if (!token) {
        alert('Please sign in so we can securely save your GPA strategy. 🔐');
        return;
    }
...
function addCourse() {
    ...
    if (!courseName) {
        alert('Add a course name first, then we can plan your path to the target GPA. 🎯');
        return;
    }
    ...
    courses.forEach((course) => {
        ...
        for (let i = 0; i < rules.ctInputs; i += 1) {
            ...
            ctHtml += `
                <div style="flex: 1; min-width: 70px;">
                    <label class="input-label ct-mark-label" style="display:block; font-size: 0.72rem; color: #8B4513; margin-bottom: 4px;">CT ${i + 1}/20</label>
                    <input
                        id="ct-${course.id}-${i}"
                        class="ct-mark-input"
                        type="number"
        ...
        card.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 12px; border-bottom: 1px solid #eee; padding-bottom: 8px;">
                <div>
                    <h3 class="course-title" style="margin:0; color:#3E2723;">${course.courseName}</h3>
                    <div class="course-meta" style="font-size:0.82rem; color:#8B4513; margin-top:4px;">Credits: ${course.credits.toFixed(2)} | Total: ${rules.totalMarks.toFixed(1)} | Final Max: ${rules.finalExamMax.toFixed(1)}</div>
                </div>
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
                <div>
                    <label class="input-label" style="display:block; font-size: 0.75rem; color: #8B4513; margin-bottom: 4px;">Attendance (${rules.attendanceMax.toFixed(1)} max)</label>
                    <input
                ...
                <div>
                    <label class="input-label" style="display:block; font-size: 0.75rem; color: #8B4513; margin-bottom: 4px;">Desired Grade</label>
            ...
            <div style="text-align:center; padding: 10px; background: #fff; border-radius: 8px; border: 1px solid #f0f0f0; margin-bottom: 12px;">
                <div id="bestCt-${course.id}" class="course-helper-text" style="font-size:0.78rem; color:#6d4c41; margin-bottom: 6px;"></div>
                <h2 id="required-${course.id}" style="margin:4px 0; font-size: 1.5rem;"></h2>
            </div>

            <div style="display:flex; justify-content:space-between; align-items:center; gap:10px;">
                <span id="saveStatus-${course.id}" class="course-helper-text" style="font-size:0.8rem; color:#6d4c41;">Not saved</span>
