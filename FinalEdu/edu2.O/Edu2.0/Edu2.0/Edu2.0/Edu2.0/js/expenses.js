...
async function fetchBackendData() {
    ...
    try {
        const response = await fetch(`http://localhost:5000/api/finance/summary?year=${currentYear}&month=${currentMonth}`, {
...
async function saveAllowance() {
    ...
    try {
        const response = await fetch('http://localhost:5000/api/finance/set-budget', {
            method: 'POST',
            headers: {
...
async function processSpending() {
    ...
    try {
        const response = await fetch('http://localhost:5000/api/finance/add-expense', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
async function deleteExpense(id) {
    ...
    try {
        const response = await fetch(`http://localhost:5000/api/finance/expense/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` }
        });
